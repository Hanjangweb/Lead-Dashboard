...
export default function Header({ toggleSidebar }) {
  ...
  const [notifs,  setNotifs]  = useState([
    { id: 1, icon: <Users size={16} />, color: "bg-indigo-100 text-indigo-600", text: "New lead added from Mumbai", time: "2m ago", read: false },
    { id: 2, icon: <TrendingUp size={16} />, color: "bg-emerald-100 text-emerald-600", text: "Conversion rate improved to 22%", time: "1h ago", read: false },
    { id: 3, icon: <BarChart2 size={16} />, color: "bg-amber-100 text-amber-600", text: "Monthly report is ready to export", time: "3h ago", read: false },
  ]);
              ...
              {notifOpen && (
                ...
                >
                  ...
                  <div className="divide-y divide-slate-50 max-h-[300px] overflow-y-auto">
                    {notifs.map(n => (
                      <div key={n.id} className={`flex items-start gap-4 px-5 py-4 hover:bg-slate-50 transition-colors cursor-pointer ${n.read ? 'opacity-50' : ''}`} onClick={() => setNotifs(prev => prev.map(item => item.id === n.id ? {...item, read: true} : item))}>
